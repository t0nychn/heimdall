# -*- coding: utf-8 -*-
"""
Created on Tue Mar 16 19:34:33 2021
...
class MySpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        # search term input
        terms = [term for term in input("请输入您的关键词（可用空格键分开每关键词做多次搜索）：").split()] 
        for term in terms:
    ...
    def parse(self, response):
        ...
        # display search info
        print("\n关键词：" + str(search_term))
        print("找到共 " + str(len(text)) + " 条结果")
        now = datetime.now()
        current_time = now.strftime("%H:%M:%S")
        print("此搜索在 " + str(date.today()) + " " + str(current_time) + " 完成")
        ...
        data = {'用户名' : author_data,
                '帖子内容' : text_data,
                '帖子信息' : info_data
                }
        df = pd.DataFrame(data, columns = ['用户名', '帖子内容', '帖子信息'])
        df.index = np.arange(1, len(df)+1)
        pd.set_option('display.max_colwidth', None)
        display(df.style.apply(highlights, subset=["帖子内容"]))
